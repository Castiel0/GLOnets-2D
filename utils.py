...
def plot_histogram(Effs,Iter, fig_path,params):
    ax = plt.figure()
    bins = [i*5 for i in range(21)]
    max=int(100*np.max(Effs))
    record=params.recorder.eff
    record=int(100*record)
    if params.gen_ver==0:
        plt.hist(Effs*100, bins, facecolor='red',edgecolor='black', alpha=0.5)
        plt.text(68,40, "{}%".format(max),fontsize=50,color='r')
    else:
        plt.hist(Effs*100, bins, facecolor='blue',edgecolor='black', alpha=0.5)
        plt.text(68,40, "{}%".format(max),fontsize=50,color='b') 
    plt.xlim(0, 100)
    plt.ylim(0, 50)
    plt.yticks([])
    ...
    plt.title('Iteration {}-Record:{}%'.format(Iter,record), fontsize=16)
...
class timer:
    ...
    def out(self):
        self.now=time.clock()
        print("Time cost:",int((self.now-self.start)/60),'min')  
        self.start=self.now  
